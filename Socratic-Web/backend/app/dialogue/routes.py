...
def handle_dialogue():
    # ... (handle_dialogue logic remains the same as v8 - saving messages) ...
    ...
    if current_user_id:  # Save history for any authenticated user
        try:
        ...
        if db_user and latest_user_message_content and ai_response_content:
            try:
                ...
                time_threshold = datetime.now(timezone.utc) - timedelta(
                    hours=current_app.config.get('CONVERSATION_CONTINUATION_THRESHOLD_HOURS', 1))

                conversation_updated_at_aware = None
                if conversation and conversation.updated_at:
                    if conversation.updated_at.tzinfo is None:
                        conversation_updated_at_aware = conversation.updated_at.replace(tzinfo=timezone.utc)
                    else:
                        conversation_updated_at_aware = conversation.updated_at

                if not conversation or (
                        conversation_updated_at_aware and conversation_updated_at_aware < time_threshold):
                    ...
                    current_app.logger.info(
                        f"POST /dialogue - Appending to existing conversation {conversation.id} for user {db_user.id}")
                ...
                db.session.commit()
                current_app.logger.info(
                    f"POST /dialogue - Saved messages to conversation {conversation.id} for user {db_user.id}")
            except Exception as e:
                db.session.rollback()
